...
import com.t1908e.WCD_assignment.util.ConvertHelper;
...
public class GenericRepository<T> implements IGenericRepository<T> {
    ...
    @Override
    public List<T> executeRawSqlQuery(String sqlQuery) {
        List<T> res = new ArrayList<T>();
        try {
            if (!isEntity()) {
                throw new EntityException("Not an entity class");
            ...
            res = fullFiled;
        } catch (EntityException | SQLException | InstantiationException | IllegalAccessException ex) {
            System.err.println("executeRawSqlQuery error: " + ex.getMessage());
        }
        return res;
    }
